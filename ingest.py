...
def enhanced_parse_timestamp_udf(date_str):
    """Parse a timestamp string without altering timezone or precision."""
    if not date_str:
        return None
    ...
    try:
        return dateutil.parser.parse(str(date_str), fuzzy=False)
    except Exception:
        try:
...
def enhanced_parse_date_udf(date_str):
    """Parse a date string preserving original granularity."""
    if not date_str:
        return None
    ...
    try:
        return dateutil.parser.parse(str(date_str), fuzzy=False).date()
    except Exception:
        return None
...
def process_table(
    ...
    try:
        ...
        if write_mode == "append":
            ...
            logger.info(
                f"Successfully wrote {final_count} rows to Snowflake for table {table_name}"
            )
